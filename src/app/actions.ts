  ...
  await prisma.$transaction(async (tx) => {
    for (const entry of entries) {
      ...
      await tx.association.create({
        data: { pairId: pair.id, direction: 'AB' },
      });
    }
  });
...
export async function saveRow(formData: FormData) {
  ...
  const parseAssociationId = (value: FormDataEntryValue | null): string | null => {
    if (typeof value !== 'string') return null;
    const trimmed = value.trim();
    return trimmed === '' ? null : trimmed;
  };

  const normalizeScoreEntry = (value: FormDataEntryValue | null): { bounded: number | null; numeric: number | null } => {
    if (value === null || value === undefined) return { bounded: null, numeric: null };
    const parsed = Number(value);
    if (Number.isNaN(parsed)) return { bounded: null, numeric: null };
    const bounded = Math.max(-1, Math.min(10, Math.round(parsed)));
    return { bounded, numeric: bounded < 0 ? null : bounded };
  };

  const associationId = parseAssociationId(formData.get('associationId'));
  const scoreEntry = normalizeScoreEntry(formData.get('score'));
  ...
  if (!pairId) {
    ...
    if (scoreEntry.numeric !== null) {
      const dueAt = new Date(Date.now() + Math.pow(5, Math.max(0, scoreEntry.numeric)) * 1000);
      await prisma.association.create({
        data: {
          pairId: created.id,
          direction: 'AB',
          score: scoreEntry.numeric,
          dueAt,
          firstTime: false,
        },
      });
    ...
    if (scoreEntry.numeric !== null) {
      ...
      await prisma.association.updateMany({
        ...
        data: {
          score: scoreEntry.numeric,
          dueAt,
          firstTime: scoreEntry.numeric === 0 ? false : undefined,
        },
      });
    } else if (scoreEntry.bounded !== null && scoreEntry.bounded < 0) {
      await prisma.association.updateMany({
        where: { id: associationId, pair: { deck: { userId } } },
        data: {
