...
type PageProps = {
  params: { slug: string; jobType: string; job: string };
};

async function fetchSponsoredJob(params: PageProps['params']) {
  const jobType = slugToJobType(params.jobType) as JobType | null;
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const job = await fetchSponsoredJob(params);
  if (!job) return {};
  ...
  return {
    title: `${job.title} at ${job.company?.name ?? 'The Student Ladder'} | The Student Ladder`,
    alternates: { canonical },
  };
}

export default async function SponsoredJobPage({ params }: PageProps) {
  const [session, job] = await Promise.all([auth(), fetchSponsoredJob(params)]);
  if (!job) {
    notFound();
  }
  ...
  return (
    <main className="page page--public">
      <article className="job-detail">
        ...
        <p>
          <a className="chip" href={job.applyUrl} target="_blank" rel="noreferrer">Apply via company site</a>
        </p>
...
function buildJobPostingJsonLd(job: NonNullable<Awaited<ReturnType<typeof fetchSponsoredJob>>>, canonical: string) {
